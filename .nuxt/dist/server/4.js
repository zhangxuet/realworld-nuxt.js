  ...
  methods: {
    async onSubmit() {
      try {
        const {
          data
        } = this.isLogin ? await Object(users["b" /* login */])({
          user: this.user
        }) : await Object(users["c" /* register */])({
          user: this.user
        });
        this.$store.commit('setUser', data.user); // 为了防止刷新页面数据丢失，需把数据持久化
